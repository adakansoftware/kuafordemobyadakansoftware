...
export function TestimonialsSection(props: TestimonialsSectionProps) {
  ...
  const testimonialCount = props.items.length
          ...
          <div className="mx-auto max-w-3xl overflow-hidden">
            <div className="rounded-[2rem] border border-border bg-card p-8 text-center shadow-[0_24px_50px_rgba(28,20,12,0.06)] md:p-12">
              <div className="mb-6 flex items-center justify-center gap-1">
                {Array.from({ length: props.items[current].rating }).map((_, index) => (
                  <Star key={index} className="h-5 w-5 fill-accent text-accent" />
                ))}
              </div>

              <p className="text-base leading-relaxed text-muted-foreground md:text-lg">{`"${props.items[current].text}"`}</p>

              <p className="mt-6 font-serif text-lg font-semibold text-foreground">{props.items[current].name}</p>
            </div>
          </div>
